...
export function ExtensionPopup() {
  return (
    <div className="w-[380px] bg-background border border-border shadow-2xl rounded-2xl overflow-hidden flex flex-col relative text-foreground font-sans">
      {/* Header */}
      <div className="h-16 border-b border-border/50 flex items-center px-4 py-3 justify-between bg-card text-foreground z-10 shadow-sm relative">
...
export function ExtensionPopupSafe() {
  return (
    <div className="w-[380px] bg-background border border-border shadow-2xl rounded-2xl overflow-hidden flex flex-col relative text-foreground font-sans">
      ...
      {/* Hero Safe Section */}
      <div className="relative p-6 pt-8 flex-col flex items-center text-center bg-gradient-to-b from-accent-green/10 to-transparent">
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          transition={{ type: "spring", bounce: 0.5 }}
          className="w-16 h-16 rounded-full bg-accent-green flex items-center justify-center text-white mb-4 shadow-[0_0_25px_hsl(var(--accent-green)/0.5)]"
        >
          <ShieldCheck size={32} />
        </motion.div>
        ...
        <div className="w-full mt-6 space-y-2">
          ...
          <div className="h-2 w-full bg-muted rounded-full overflow-hidden">
            <motion.div
              ...
              animate={{ width: "3%" }}
              transition={{ duration: 1, delay: 0.3, ease: "easeOut" }}
              className="h-full bg-accent-green relative"
            >
              <div className="absolute top-0 right-0 w-8 h-full bg-white/30 blur-sm transform skew-x-12" />
            </motion.div>
          </div>
        </div>
      </div>
        ...
        <ul className="space-y-3">
          {[
            ...
            <motion.li
              ...
              animate={{ opacity: 1, x: 0 }}
              transition={{ delay: 0.6 + i * 0.1 }}
              className="flex items-start gap-2 text-sm bg-accent-green/5 p-2 rounded border border-accent-green/10"
            >
